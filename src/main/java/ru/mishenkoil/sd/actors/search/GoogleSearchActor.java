...
public class GoogleSearchActor extends SearchActor {

    public static Behavior<MakeRequest> create() {
        return Behaviors.setup(GoogleSearchActor::new);
    }

    public GoogleSearchActor(ActorContext<MakeRequest> context) {
        super(context);
    }
    ...
    protected Behavior<MakeRequest> onMakeRequest(MakeRequest makeRequest) throws IOException, InterruptedException {
        ...
        var request = HttpRequest
                .newBuilder(URI.create("http://localhost:8091/search?q=" + makeRequest.query()))
